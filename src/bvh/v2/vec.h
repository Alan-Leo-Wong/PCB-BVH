...
namespace bvh::v2 {
    ...
    struct Vec {
        ...
        template<typename T, size_t N>
        BVH_ALWAYS_INLINE bool operator==(const Vec<T, N> &other) const {
    ...
    // modified
    template<typename T, size_t N>
    BVH_ALWAYS_INLINE auto cross(const Vec<T, N> &a, const Vec<T, N> &b);

    template<typename T>
    BVH_ALWAYS_INLINE T cross(const Vec<T, 2> &a, const Vec<T, 2> &b) {
