...
template <typename T, size_t N> struct PCBData {
  /// type
  using Point = Vec<T, N>;
...
template <typename T, size_t N> struct PCBSeg : public PCBData<T, N> {
...
template <typename T, size_t N> struct ArcData {
  /// data
  Vec<T, N> center;
  T radius = (T)NAN;
...
template <typename T, size_t N> struct PCBArc : public PCBData<T, N> {
  using typename PCBData<T, N>::Point;
