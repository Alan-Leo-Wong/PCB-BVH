...
/// contains. By definition, inner BVH nodes do not contain primitives; only leaves do.
    ...
    struct Node {
        ...
        BVH_ALWAYS_INLINE Vec<T, Dim> get_max_bounds(const Octant &octant) const {
            return Vec<T, Dim>::generate(
                    [&](size_t i) { return bounds[2 * static_cast<uint32_t>(i) + 1 - octant[i]]; });
        }
        ...
        BVH_ALWAYS_INLINE bool is_intersect_bbox(const BBox<T, Dim> &_bbox) const {
            auto bbox = get_bbox();

            for (size_t i = 0; i < Dim; ++i) {
                if (bbox.min[i] > _bbox.max[i] ||
                    bbox.max[i] < _bbox.min[i])
                    return false;
            }
            return true;
        }

        BVH_ALWAYS_INLINE std::pair<T, bool> get_square_dis(const Vec<T, Dim> &query) const {
            auto bbox = get_bbox();

            /*std::cout << "==============\nbvh bbox:\n";
            std::cout << "min: " << bbox.min[0] << ", " << bbox.min[1] << std::endl;
            std::cout << "max: " << bbox.max[0] << ", " << bbox.max[1] << std::endl;*/

            bool inside = true;
            T min_border_dis = std::numeric_limits<T>::max();
            T sqr_dis = 0;
            for (size_t i = 0; i < Dim; ++i) {
                if (query[i] < bbox.min[i]) {
                    T diff = bbox.min[i] - query[i];
                    sqr_dis += diff * diff;
                    if (inside) inside = false;
                } else if (query[i] > bbox.max[i]) {
                    T diff = query[i] - bbox.max[i];
                    sqr_dis += diff * diff;
                    if (inside) inside = false;
                }
                /*else {
                    T diff = std::min(query[i] - bbox.min[i], bbox.max[i] - query[i]);
                    min_border_dis = std::min(min_border_dis, diff * diff);
                }*/
            }

            // if (inside) sqr_dis = min_border_dis;

            /*std::cout << "dis: " << std::sqrt(sqr_dis) << std::endl;
            std::cout << "inside: " << inside << std::endl;*/

            return std::pair{sqr_dis, inside};
        }
